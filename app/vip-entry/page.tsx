...
export default function VipEntryPage() {
  ...
  // 用来做「有没有传东西」的统一判断
  const entryKey = qrCodeParam || tokenParam || vipKeyParam || "";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setTimeout(() => {
          router.replace(
            `/vip-chat?sessionId=${encodeURIComponent(data.sessionId!)}`
          );
        }, 500);
      } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#fbf3e7] flex justify-center">
      <div className="w-full max-w-md flex flex-col bg-[#fbf3e7]">
        ...
        {/* 内容区 */}
        <div className="flex-1 px-7 pt-8 pb-12 flex flex-col items-center">
          <h1 className="w-full text-[20px] font-semibold text-[#3a3023] mb-2">
            Wynn Palace · VIP Concierge
          </h1>

          <p className="w-full text-[12px] text-[#6e5842] mb-6 leading-relaxed">
          ...
          <div className="w-full flex flex-col items-center">
            ...
            {/* 状态区：避免“重复/闪动”——idle/processing 只显示副标题；success/error 才显示 message */}
            <div className="mt-8 text-center px-4">
              <div className="text-[13px] tracking-[0.22em] font-semibold text-[#333333] uppercase">
                {statusLine}
                {isProcessing && (
              ...
              <div className="mt-[6px] text-[11px] text-[#9a7a5c]">
                {subStatusLine}
              </div>
            ...
            <div className="mt-6 text-[10px] text-[#9a856a] text-center">
